...
import com.nacos.generator.controller.request.TableGenInfo;
import com.nacos.generator.controller.request.TableInfo;
import com.nacos.generator.util.entity.GenDto;
...
public class Freemarker {
     ...
     * @throws TemplateException
     */
    public void controller(TableInfo tableInfo, Map<String,String> table, List<Map<String,String>> tableInfos) throws IOException, TemplateException {
        // 第一步：创建一个Configuration对象，直接new一个对象。构造方法的参数就是freemarker对于的版本号。
        Configuration configuration = new Configuration(Configuration.getVersion());
        File file = ResourceUtils.getFile(ResourceUtils.CLASSPATH_URL_PREFIX+"templates");
        // 第二步：设置模板文件所在的路径。
     ...
     * @throws TemplateException
     */
    public void web(TableInfo tableInfo, Map<String,String> table, List<Map<String,String>> tableInfos) throws IOException, TemplateException {
        // 第一步：创建一个Configuration对象，直接new一个对象。构造方法的参数就是freemarker对于的版本号。
        Configuration configuration = new Configuration(Configuration.getVersion());
        File file = ResourceUtils.getFile(ResourceUtils.CLASSPATH_URL_PREFIX+"templates");
        // 第二步：设置模板文件所在的路径。
     ...
     * @throws TemplateException
     */
    public void webjs(TableInfo tableInfo, Map<String,String> table, List<Map<String,String>> tableInfos) throws IOException, TemplateException {
        // 第一步：创建一个Configuration对象，直接new一个对象。构造方法的参数就是freemarker对于的版本号。
        Configuration configuration = new Configuration(Configuration.getVersion());
        File file = ResourceUtils.getFile(ResourceUtils.CLASSPATH_URL_PREFIX+"templates");
        // 第二步：设置模板文件所在的路径。
        ...
        configuration.setDefaultEncoding("utf-8");
        // 第四步：加载一个模板，创建一个模板对象。
        Template template = configuration.getTemplate("webjs.ftl");
        // 第五步：创建一个模板使用的数据集，可以是pojo也可以是map。一般是Map。
        Map dataModel = new HashMap<>();
        ...
        // 第六步：创建一个Writer对象，一般创建一FileWriter对象，指定生成的文件名。
        Writer out = new FileWriter(new File(gearfileUrl+dataModel.get("smClassName")+".js"));
        // 第七步：调用模板对象的process方法输出文件。
        template.process(dataModel, out);
        // 第八步：关闭流。
        out.close();
